...
# Create output directory and give your own path for output directory where you want to save the results
output_dir = r'G:\IIT_AI work\ISRAEL\GWS_Model info\Data and feature score\Revised data and feature_fitting scores\Final results (dataset_28.8)\Israel model run\LR_country_final_single_run1' # used in my case
...
sheet_name = "Sheet1"
# Load your data and give your own path in #excelFilePath, where the data shared is been saved
...
def create_actual_vs_predicted_plot(y_test, y_pred, metrics, output_dir, model_name="Linear Regression"):
    """
...
# =============================================
# SINGLE RUN TRAINING FUNCTION WITH LINEAR REGRESSION
# =============================================

def train_single_run_with_density_weights(X, y, selected_columns, basin_densities, random_state=42):
    """
    Train Linear Regression model on a single run with basin density weighting using inverse square-root method
    """
    ...
    # Train final Linear Regression model with sample weights
    print("Training Linear Regression model with basin density weights...")
    ...
    print("\n" + "=" * 50)
    print("LINEAR REGRESSION MODEL PERFORMANCE RESULTS")
    print("=" * 50)
    print(f"MSE: {mse:.4f}")
    ...
    additional_metrics = create_actual_vs_predicted_plot(
        y_test, y_pred, metrics_dict, single_run_dir, "Linear Regression"
    )
    ...
    print("Performing SHAP analysis for Linear Regression...")
    try:
        # Use LinearExplainer for Linear Regression
        explainer = shap.LinearExplainer(model, X_train_selected, feature_names=selected_features)
    ...
    # Additional Linear Regression specific diagnostics
...
# =============================================
# MAIN EXECUTION - SINGLE RUN WITH LINEAR REGRESSION
# =============================================

if __name__ == "__main__":
    print("Starting single run with Linear Regression and basin density weighting...")
    print("=" * 60)
    ...
    if results:
        ...
        if os.path.exists(feature_importance_path):
            ...
            for idx, row in top_features.iterrows():
                print(
                    f"{idx + 1:2d}. {row['feature']}: Importance = {row['importance']:.4f}, Coef = {row['coefficient']:.4f}")
